...
PR(_veh) = (nearestObjects [player,["LandVehicle","Air","Ship"], DISTANCE_TO_REPAIRVEHICLE]) select 0;
if (isNil {_veh}) exitWith {};
if (WMT_mutexAction) exitWith {
	localize("STR_ANOTHER_ACTION") call WMT_fnc_NotifyText;
};

if (not alive player or (player distance _veh) > DISTANCE_TO_REPAIRVEHICLE or (vehicle player != player) or speed _veh > 3) exitWith {localize("STR_REPAIR_CONDITIONS") call WMT_fnc_NotifyText;};
